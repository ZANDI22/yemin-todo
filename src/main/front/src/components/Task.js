...
function Task({ task }) {
    ...
    const waitForAnimationAndRemove = (id) => {
        setTimeout(() => {
            removeTask(id);
        }, 250);
    };

    const handleUpdateDate = (task, date) => {
    ...
    return (
        ...
        >
            ...
            <div className="right">
                <h4 className="date" onClick={() => setIsSelectingDate(!isSelectingDate)}>{`${task.date.split(" ")[1]} ${task.date.split(" ")[2]
                    }`}</h4>
